...
- **`publish_xhs.md`**：仅在**已填写「发布命令」且开启「启用外部发布脚本」**、需要把环境变量交给本机脚本时才会被读取。
...
- **启用外部发布脚本**：开启后，「发布小红书」在渲染完图片后执行你填的**本机命令**。
- **发布命令** / **Dry-run**：见下文环境变量表。

脚本会收到环境变量（与 `scripts/publish_xhs_stub.js` 一致）：
...
4. **小红书**：准备 `xhs_content.md` 后，点 **小红书** 或 `MDTP：发布小红书`（未配置发布命令时也会先渲染 PNG）；只出图、不走脚本可用 `MDTP：仅渲染小红书卡片图`。
